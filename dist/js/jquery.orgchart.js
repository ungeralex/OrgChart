...
(function (factory) {
    ...
    //
    let OrgChart = function (elem, opts) {
    ...
    OrgChart.prototype = {
            ...
            if (this.options.exportButton && !$chartContainer.find('.oc-export-btn').length) {
                let $exportBtn = $('<button>', {
                    ...
                    'click': function(e) {
                        ...
                        let $mask = $chartContainer.find('.mask');
                        if (!$mask.length) {
                            $chartContainer.append('<div class="mask"><i class="fa fa-circle-o-notch fa-spin spinner"></i></div>');
                        } else {
                            $mask.removeClass('hidden');
                        }
                        let sourceChart = $chartContainer.addClass('canvasContainer').find('.orgchart:visible').get(0);
                        let flag = that.options.direction === 'l2r' || that.options.direction === 'r2l';
                        html2canvas(sourceChart, {
                            ...
                            'onrendered': function(canvas) {
                                ...
                                if (that.options.exportFileextension.toLowerCase() === 'pdf') {
                                    let doc = {};
                                    let docWidth = Math.floor(canvas.width * 0.2646);
                                    let docHeight = Math.floor(canvas.height * 0.2646);
                                    ...
                                    let isFf = !!window.sidebar;
                                    let isEdge = navigator.appName === 'Microsoft Internet Explorer' || (navigator.appName === "Netscape" && navigator.appVersion.indexOf('Edge') > -1);
                ...
                if (this.options.exportFileextension.toLowerCase() !== 'pdf') {
                    let downloadBtn = '<a class="oc-download-btn' + (this.options.chartClass !== '' ? ' ' + this.options.chartClass : '') + '"'
        ...
        zoomWheelHandler (e) {
            let oc = e.data.oc;
            e.preventDefault();
            let newScale  = 1 + (e.originalEvent.deltaY > 0 ? -0.2 : 0.2);
        ...
        zoomEndHandler (e) {
            ...
            if(oc.$chart.data('pinching')) {
                ...
                let diff = oc.$chart.data('pinchDistEnd') - oc.$chart.data('pinchDistStart');
                if (diff > 0) {
        ...
        loopChart ($chart) {
            let that = this;
            let $tr = $chart.find('tr:first');
            let subObj = { 'id': $tr.find('.node')[0].id };
        ...
        hideParent ($node) {
            let $temp = $node.closest('table').closest('tr').siblings();
            ...
            let $parent = $temp.eq(0).find('.node');
            let grandfatherVisible = this.getNodeState($parent, 'parent').visible;
        ...
        showParent ($node) {
            ...
            let $temp = $node.closest('table').closest('tr').siblings().removeClass('hidden');
            // just show only one line
            $temp.eq(2).children().slice(1, -1).addClass('hidden');
            // show parent node with animation
            let parent = $temp.eq(0).find('.node')[0];
            this.repaint(parent);
        ...
        hideChildren ($node) {
            let that = this;
            let $temp = $node.closest('tr').siblings();
            if ($temp.last().find('.spinner').length) {
                $node.closest('.orgchart').data('inAjax', false);
            }
            let $visibleNodes = $temp.last().find('.node:visible');
            let isVerticalDesc = $temp.last().is('.verticalNodes') ? true : false;
            if (!isVerticalDesc) {
                let $lines = $visibleNodes.closest('table').closest('tr').prevAll('.lines').css('visibility', 'hidden');
            }
        ...
        showChildren ($node) {
            let that = this;
            let $levels = $node.closest('tr').siblings();
            let isVerticalDesc = $levels.is('.verticalNodes') ? true : false;
            let $descendants = isVerticalDesc
        ...
        hideSiblings ($node, direction) {
            let that = this;
            let $nodeContainer = $node.closest('table').parent();
            ...
            let $animatedNodes = $nodeContainer.siblings().find('.slide');
            let $lines = $animatedNodes.closest('.nodes').prevAll('.lines').css('visibility', 'hidden');
            $animatedNodes.eq(0).one('transitionend', function() {
                $lines.removeAttr('style');
                let $siblings = direction ? (direction === 'left' ? $nodeContainer.prevAll(':not(.hidden)') : $nodeContainer.nextAll(':not(.hidden)')) : $nodeContainer.siblings();
        ...
        showSiblings ($node, direction) {
            ...
            let $upperLevel = $node.closest('table').closest('tr').siblings();
            if (direction) {
            ...
            if (!this.getNodeState($node, 'parent').visible) {
                ...
                let parent = $upperLevel.find('.node')[0];
                this.repaint(parent);
                $(parent).addClass('slide').removeClass('slide-down').one('transitionend', function() {
        ...
        startLoading ($arrow, $node, options) {
            let $chart = $node.closest('.orgchart');
        ...
        endLoading ($arrow, $node, options) {
            let $chart = $node.closest('div.orgchart');
        ...
        switchHorizontalArrow ($node) {
            let opts = $node.closest('.orgchart').data('options');
            if (opts.toggleSiblingsResp && (typeof opts.ajaxURL === 'undefined' || $node.closest('.nodes').data('siblingsLoaded'))) {
                let $prevSib = $node.closest('table').parent().prev();
                if ($prevSib.length) {
                ...
                let $nextSib = $node.closest('table').parent().next();
                if ($nextSib.length) {
                ...
                let $sibs = $node.closest('table').parent().siblings();
                let sibsVisible = $sibs.length ? !$sibs.is('.hidden') : false;
        ...
        createNode(nodeData, level, opts) {
            ...
            let $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + '>')
            ...
            if (opts.verticalDepth && (level + 2) > opts.verticalDepth) {
                if ((level + 1) >= opts.verticalDepth && Number(flags.substr(2,1))) {
                    let icon = level + 1  >= opts.depth ? 'plus' : 'minus';
            ...
            $nodeDiv.on('mouseenter mouseleave', function(event) {
                let $node = $(this), flag = false;
                let $topEdge = $node.children('.topEdge');
                let $rightEdge = $node.children('.rightEdge');
                let $bottomEdge = $node.children('.bottomEdge');
                let $leftEdge = $node.children('.leftEdge');
            ...
            $nodeDiv.on('click', '.topEdge', function(event) {
                ...
                let parentState = that.getNodeState($node, 'parent');
                if (parentState.exist) {
                    let $parent = $node.closest('table').closest('tr').siblings(':first').find('.node');
            ...
            $nodeDiv.on('click', '.bottomEdge', function(event) {
                ...
                let $that = $(this);
                let $node = $that.parent();
                let childrenState = that.getNodeState($node, 'children');
                if (childrenState.exist) {
                    let $children = $node.closest('tr').siblings(':last');
            ...
            $nodeDiv.on('click', '.toggleBtn', function(event) {
                let $this = $(this);
                let $descWrapper = $this.parent().next();
                let $descendants = $descWrapper.find('.node');
                let $children = $descWrapper.children().children('.node');
            ...
            $nodeDiv.on('click', '.leftEdge, .rightEdge', function(event) {
                ...
                let $that = $(this);
                let $node = $that.parent();
                let siblingsState = that.getNodeState($node, 'siblings');
                if (siblingsState.exist) {
                    let $siblings = $node.closest('table').parent().siblings();
                    ...
                    let nodeId = $that.parent()[0].id;
                    let url = (that.getNodeState($node, 'parent').exist) ?
            ...
            if (opts.draggable) {
                $nodeDiv.on('dragstart', function(event) {
                    let origEvent = event.originalEvent;
                    ...
                    if ($nodeDiv.closest('.orgchart').css('transform') !== 'none') {
                        ...
                        let transValues = $nodeDiv.closest('.orgchart').css('transform').split(',');
                        let scale = Math.abs(window.parseFloat((opts.direction === 't2b' || opts.direction === 'b2t') ? transValues[0].slice(transValues[0].indexOf('(') + 1) : transValues[1]));
                    ...
                    let $dragged = $(this);
                    let $dragZone = $dragged.closest('.nodes').siblings().eq(0).find('.node:first');
                    let $dragHier = $dragged.closest('table').find('.node');
                    ...
                    .on('drop', function(event) {
                        let $dropZone = $(this);
                        let $orgchart = $dropZone.closest('.orgchart');
                        let $dragged = $orgchart.data('dragged');
                        $orgchart.find('.allowedDrop').removeClass('allowedDrop');
                        let $dragZone = $dragged.closest('.nodes').siblings().eq(0).children();
                        ...
                        if (!$dropZone.closest('tr').siblings().length) { // if the drop zone is a leaf node
                            ...
                            let dropColspan = parseInt($dropZone.parent().attr('colspan')) + 2;
                            let horizontalEdges = '<i class="edge horizontalEdge rightEdge fa"></i><i class="edge horizontalEdge leftEdge fa"></i>';
                            ...
                            let $dropSibs = $dragged.closest('table').parent().siblings().find('.node:first');
                            if ($dropSibs.length === 1) {
                                $dropSibs.append(horizontalEdges);
                            }
                        }
                        // secondly, deal with the hierarchy of dragged node
                        let dragColspan = parseInt($dragZone.attr('colspan'));
                        if (dragColspan > 2) {
                            $dragZone.attr('colspan', dragColspan - 2)
                                .parent().next().children().attr('colspan', dragColspan - 2)
                                .end().next().children().slice(1, 3).remove();
                            let $dragSibs = $dragZone.parent().siblings('.nodes').children().find('.node:first');
        ...
        buildHierarchy ($appendTo, nodeData, level, opts, callback) {
            ...
            let $nodeWrapper;
            // Construct the node
            let $childNodes = nodeData.children;
            let hasChildren = $childNodes ? $childNodes.length : false;
            let isVerticalNode = (opts.verticalDepth && (level + 1) >= opts.verticalDepth) ? true : false;
            ...
            if (hasChildren) {
                ...
                let isHidden = (level + 1 >= opts.depth || nodeData.collapsed) ? ' hidden' : '';
                let isVerticalLayer = (opts.verticalDepth && (level + 2) >= opts.verticalDepth) ? true : false;
                ...
                let lineLayer = '<tr class="lines' + isHidden + '"><td class="rightLine">&nbsp;</td>';
                ...
                $.each($childNodes, function() {
                    let $nodeCell = isVerticalLayer ? $('<li>') : $('<td colspan="2">');
        ...
        buildParentNode ($currentRoot, nodeData, opts, callback) {
            ...
            $.when(this.createNode(nodeData, 0, opts || $currentRoot.closest('.orgchart').data('options')))
                .done(function($nodeDiv) {
                    ...
                    let linesRow = '<td class="rightLine">&nbsp;</td><td class="leftLine">&nbsp;</td>';
        ...
        buildSiblingNode ($nodeChart, nodeData, opts, callback) {
            ...
            let insertPostion = (siblingCount > 1) ? Math.floor(siblingCount/2 - 1) : 0;
        ...
        //
        removeNodes ($node) {
            let $parent = $node.closest('table').parent();
            let $sibs = $parent.parent().siblings();
            if ($parent.is('td')) {
